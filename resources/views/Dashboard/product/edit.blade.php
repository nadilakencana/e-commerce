        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-content">
                    <div class="card-body">
                        <form class="form" action="{{ route('update-product', encrypt($product->id)) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select  id="last-name-column" class="form-control @error('id_category') is-invalid @enderror"
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea type="text" id="last-name-column" class="form-control @error('deskripsi') is-invalid @enderror"
                                            placeholder="Product Description" name="deskripsi">{{ $product->deskripsi }}</textarea>
                                           @error('deskripsi')
                                                <small class="invalid-feedback">{{ $message }}</small>
                                            @enderror
                                    </div>
                                    ...
                                    <div class="varian-color mt-3">
                                        @foreach ($var_color as $k => $color )
                                            <div class="itm-color" xid="{{ $k }}">
                                                <div class="btn btn-danger item-color" xid="{{ $k }}"><i class="bi bi-trash"></i></div>
                                                <input type="text" class="form-control color @error('name') is-invalid @enderror"
                                                 name="color[{{ $k }}][name]" xid="{{ $k }}" value="{{ $color->name }}">
                                                <input type="hidden" name="color[{{ $k }}][id]" value="{{ $color->id }}">
                                                @error("name")
                                                    <small class="invalid-feedback">{{ $message }}</small>
                                                @enderror
                                            </div>

                                            <input type="text" name="color[{{ $k }}][id]" value="{{ $color->id }}" xid="{{ $k }}" style="display: none">
                                            <input type="text"  name="color[{{ $k }}][name]" xid="{{ $k }}" value="{{ $color->name }}" style="display: none">
                                        @endforeach
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    ...
                                    <div class="varian-size mt-3">
                                         @foreach ($var_size as $j => $size )
                                            <div class="itm-size" xid="{{ $j }}">
                                                <div class="btn btn-danger item-size" xid="{{ $j }}"><i class="bi bi-trash"></i></div>
                                                <input type="text" class="form-control size @error('ukuran') is-invalid @enderror"
                                                 name="size[{{ $j }}][ukuran]" xid="{{ $j }}" value="{{ $size->ukuran }}">
                                                <input type="hidden" name="size[{{ $j }}][id]" value="{{ $size->id }}">
                                                @error("ukuran")
                                                    <small class="invalid-feedback">{{ $message }}</small>
                                                @enderror
                                            </div>
                                            <input type="text" name="size[{{ $j }}][id]" value="{{ $size->id }}" xid="{{ $j }}" style="display: none">
                                            <input type="text"  name="size[{{ $j }}][ukuran]" xid="{{ $j }}" value="{{ $size->ukuran }}" style="display: none">
        ...
        function addColor(xid){
            ...
            $target.append(
                `<div class="itm-color" xid="${xid}">` +
                     ...
                     `<input type="text" class="form-control @error('name') is-invalid @enderror" name="color[${xid}][name]" xid="${xid}">`+
                     '@error("name")'+
                         '<small class="invalid-feedback">{{ $message }}</small>'+
                     '@enderror'+
                '</div>'
        ...
        function addSize(xid){
            ...
            $target.append(
                `<div class="itm-size" xid="${xid}">` +
                     ...
                     `<input type="text" class="form-control @error('ukuran') is-invalid @enderror" name="size[${xid}][ukuran]" xid="${xid}">`+
                     '@error("ukuran")'+
                         '<small class="invalid-feedback">{{ $message }}</small>'+
                     '@enderror'+
                '</div>'
