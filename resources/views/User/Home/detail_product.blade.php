            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="varian-warna d-flex gap-3 align-items-center mb-3">
                        ...
                        <button class="btn btn-outline-dark color" xid = "{{ $color->id }}" onclick="btnActiveOption(this, 'color')">{{ $color->name }}</button>
                        @endforeach
                    ...
                    <div class="varian-ukuran d-flex gap-3 align-items-center mb-3">
                        ...
                        <button class="btn btn-outline-dark size" xid= " {{ $size->id }}" onclick="btnActiveOption(this, 'size')">{{ $size->ukuran }}</button>
                        @endforeach
                    </div>
                    ...
                    <div class="footer-card d-flex justify-content-between gap-4">
                        <div class="qty-controll qty" id="qty">
                        ...
                        <div class="card p-2">
                            <div class="card-content">
                                ...
                                <div class="title-card">
                                    <a href="{{ route('detail-product', $pro->slug) }}" class="cl-dark">{{ $pro->name
                                        }}</a>
                                </div>
...
<script>
    ...
    function addToCart(id){
        var qty =  $('.qty-controll .qty-input').val();
        var id_warna = $('.varian-warna .color.active-btn').attr('xid');
        var warna = $('.varian-warna .color.active-btn').text();
        var id_ukuran= $('.varian-ukuran .size.active-btn').attr('xid');
        var ukuran = $('.varian-ukuran .size.active-btn').text();
    ...
    function btnActiveOption(button, type){
        ...
        for(var i = 0; i< button.length; i++){
            if(buttons[i] !== button){
                buttons[i].classList.remove('action-btn');
            }
        }
    }
</script>
